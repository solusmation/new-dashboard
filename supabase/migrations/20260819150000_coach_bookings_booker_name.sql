-- Add booker_name column for admin-created bookings
ALTER TABLE public.coach_bookings
  ADD COLUMN IF NOT EXISTS booker_name text;

-- Update the hub grid RPC to prefer booker_name over profile name
...
BEGIN
  ...
  FOR v_court IN 1..8 LOOP
    FOR v_h IN v_hour_start .. v_hour_end - 1 LOOP
      ...
      SELECT cb.id, cb.duration_hours, cb.coach_fee_idr,
             COALESCE(cb.booker_name, p.display_name) AS display_name,
             p.username
